...
// 返回顶部
    ...
    $(window).scroll(function () {
        ...
        if (scrollTop > screemHeight) {
            $(".back-top-ctr").fadeIn();
        } else if (scrollTop <= screemHeight) {
            $(".back-top-ctr").fadeOut();
        }

    });
    $('.back-top-ctr .back-top-btn').click(function () {
        var scrollTop = parseFloat($(this).attr('data-scrollTop')) || 0
    ...
    // 设置图片大小为正方形
    function autoSizeBox(selector) {
        var screemWidth = $(".BuyRecord-box").width();
        var elmheight = 49/100 * screemWidth - 1/100 * screemWidth - 10
        $("body").before("<style>.img-square{height:" + elmheight + "px;}</style>")
    }

    autoSizeBox(".img-square")
...
//手机号中间--星号代替
function cutPhone(teltxt) {
    var teltxt = $(teltxt);
    var changeTel = teltxt.html().substring(0, 3) + "****" + teltxt.html().substring(7, 11);
    return changeTel;
}
...
function NoEmpty() {
    var is_empty = false;
    var txtTips = "";
    $(".mail li").each(function () {
        if ($(this).find("input").val() == "") {
            txtTips = $(this).find("span").text();
            alertTx(txtTips + "不能为空");
            is_empty = true;
            return false;
        }
    })
...
//弹框（两个按钮）
function alertTxDouble(msg, callback) {
    Scrollheight()
    $('body').append($('<div class="prompt-box"><div><div class="Close-Right"></div><p>' + msg + '</p><div class="prompt-double-btn"><button class="hidden">否</button><button class="show">是</button></div></div></div>'));
    $(".hidden").on('click', function () {
        $('.prompt-box').hide();
        unScrollheight()
        if (callback) {
...
/*新增弹框*/
function warn_alertlayer() {
    Scrollheight();
    $('.warn-alertlayer').show();
}

function unwarn_alertlayer() {
    $('.warn-alertlayer').hide();
    unScrollheight();
}
...
// 音乐自动播放 适用于微信浏览器
function autoPlayAudio(selector) {
...
var doscroll = function () {
    ...
    var boxHeight = $(".slide-list").height();
    if (idx == boxHeight / 30) {
        $first.clone().appendTo($parent);
        $parent.animate({
            marginTop: -30 * idx + 'px'
        }, 500, function () {
            $parent.css({marginTop: -0 + 'px'});
            idx = 1

...
function mLine(divId) {
  ...
  var ZoomStep = (maxTime - minTime) / 6
  ...
  var mychart = Highcharts.StockChart({
      ...
      events: {
        load: function () {
          ...
          setInterval(function () {
            ...
            y0 = Math.random()*80 - Math.random()*80 + 3500.99;
            y1 = Math.random() - Math.random() + 2500;
            series0.addPoint([x, y0], false);
            series1.addPoint([x, 2500], true);
            idx++;
            ...
            $(".zoom-ctr .to-large").on('click', function(event) {
                console.log("变大")
            ...
            $(".zoom-ctr .to-small").on('click', function(event) {
                console.log("缩小")
                minTime = minTime - ZoomStep
                maxTime = maxTime + ZoomStep
            ...
            $(".zoom-ctr .to-left").on('click', function(event) {
                console.log("向左")
                if (minTime > 1489023000000) {
                  minTime = minTime - ZoomStep
                  maxTime = maxTime - ZoomStep
                }else{
            ...
            $(".zoom-ctr .to-right").on('click', function(event) {
                console.log("向右")
                if (maxTime  < 1489042800000) {
                  minTime = minTime + ZoomStep
                  maxTime = maxTime + ZoomStep
                }else{
            ...
            $(".zoom-ctr .to-restore").on('click', function(event) {
                console.log("复原")
                minTime = 1489023000000
                maxTime = 1489042800000
                mychart.xAxis[0].update({
            ...
            //     console.log("变大")
            //     mychart.xAxis[0].update({
            //       min: minTime + (maxTime-minTime)/4, //1489023000000,
            //       max: maxTime - (maxTime-minTime)/4, //1489042800000,
            //     });
            // });
            ...
            //     console.log("缩小")
            //     mychart.xAxis[0].update({
            //       min: minTime + (maxTime-minTime)/4 - (maxTime-minTime)/4, //1489023000000,
            //       max: maxTime - (maxTime-minTime)/4 + (maxTime-minTime)/4, //1489042800000,
            //     });
            // });
            ...
            //     console.log("向右")
            //     mychart.xAxis[0].update({
            //       min: minTime + (maxTime-minTime)/4 + (maxTime-minTime)/4 + (maxTime-minTime)/4, //1489023000000,
            //       max: maxTime - (maxTime-minTime)/4 + (maxTime-minTime)/4 + (maxTime-minTime)/4, //1489042800000,
            //     });
            // });
    ...
    tooltip: {
      positioner: function () { //设置tips显示的相对位置
        ...
        var height = this.chart.chartHeight / 5 - 40;//chart高度
        if (mouseX < halfWidth) {
